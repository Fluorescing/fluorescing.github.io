...
class SyrupDemo {
    ...
    DoSimulation(rotation, polangle) {
        ...
        // get the intensity
        return Math.sqrt(
            H.y().re * H.y().re + H.y().im * H.y().im +
            H.x().re * H.x().re + H.x().im * H.x().im);
    }

    update(ctx, angle, polfilter, redfilter, greenfilter, bluefilter) {
        ...
        for (var x = 0; x < this.WIDTH; x++) {
            ...
            if (!redfilter) red *= 0;
            if (!greenfilter) green *= 0;
            if (!bluefilter) blue *= 0;
    ...
    static Rotator(v, phase) {
        var cos = new Complex(Math.cos(phase), 0);
        var sin = new Complex(Math.sin(phase), 0);
        return new CVec([
            Complex.add(Complex.mul(v.x(), cos), Complex.mul(v.y(), sin)),
            Complex.sub(Complex.mul(v.x(), sin), Complex.mul(v.y(), cos))
        ]);
    }
}
